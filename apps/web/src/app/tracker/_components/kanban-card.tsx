...
const WorkLocationBadge = ({ type }: { type: Application["remote"] }) => {
    const config = {
        ONSITE: { icon: Building, label: "Onsite", className: "bg-orange-100 text-orange-700 dark:bg-orange-950 dark:text-orange-300" },
        REMOTE: { icon: Globe, label: "Remote", className: "bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300" },
        HYBRID: { icon: Laptop, label: "Hybrid", className: "bg-blue-100 text-blue-700 dark:bg-blue-950 dark:text-blue-300" },
    };

    const { icon: Icon, label, className } = config[type];

    return (
        <span className={cn(`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium`, className)}>
...
export function KanbanCard({ application, index }: KanbanCardProps) {
    const { company, position, location, remote, minSalary, maxSalary, salaryCurrency } = application;
    const salaryDisplay = formatSalary(minSalary, maxSalary, salaryCurrency);

    return (
        <Draggable draggableId={`app-${application.id}`} index={index}>
            {(provided, snapshot) => (
                <div
                ...
                >
                    ...
                    <div className="flex items-start gap-3 mb-3">
                        <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-muted flex items-center justify-center overflow-hidden ring-1 ring-border">
                            ...
                            <Building2 className={`w-5 h-5 text-muted-foreground ${company.logoUrl ? "hidden" : ""}`} />
                        </div>
